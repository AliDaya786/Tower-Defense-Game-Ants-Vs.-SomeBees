...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while tempplace.is_hive != True:
            randombee = random_bee(tempplace.bees)
            if randombee != None and self.min_range <= counter <= self.max_range:
                return random_bee(tempplace.bees)
            else:
                tempplace = tempplace.entrance #switch tempplace with the place in front of it
                counter+=1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0: #If FireAnt explodes
            explodedamage = amount + self.damage
            for bee in copy_bees:
                bee.reduce_health(explodedamage)
        else:
            for bee in copy_bees:
...
class WallAnt(Ant):
    ...
    def __init__(self, health=4):
        """Create an WallAnt with a HEALTH quantity."""
        super().__init__(health)
# END Problem 6
    ...
    def slow(self, length):
        ...
        def action(gamestate):
            if gamestate.time % 2 == 0:
                self.action(gamestate) #action(gamestate)
        return action
        # END Problem EC
